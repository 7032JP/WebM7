...
書き込んだ T77 / WAV はそのまま LOAD で読み戻せます（往復可能）。
...
1. **ROM Files** で必要な ROM を読み込む。**最低限**:
   - `FBASIC30.ROM`
   - サブシステム ROM（`SUBSYS_C.ROM` など）
   - ブート ROM（FM-7 系は `BOOT_BAS.ROM`、FM77AV 系は `INITIATE.ROM`）
2. ヘッダーの**機種タブ**で機種を選択（FM-7 / FM77AV / FM77AV20 / FM77AV40 / FM77AV20EX / FM77AV40EX）。
3. 機種別ハードウェアパネルで**起動モード**を **BASIC** に設定。
4. ハードウェアパネルの**電源スイッチ**を押す。`OK` プロンプト（F-BASIC READY）が表示されるまで待つ。

> 電源・リセット・起動モードは、実機の筐体を模した**ハードウェアパネル**にあります。操作部の位置はスキン（見た目）によって変わります。詳しくは[チュートリアル](Tutorial.md)の「4. 起動する」を参照してください。

> BASIC キーワードは**大文字**で入力してください。BASIC Paste で `.txt` を流し込む場合も
> 中身は大文字推奨です（小文字キーワードは F-BASIC がエラーにします）。
...
2. 品質表示が **`N blocks OK`（BAD なし）**なら成功です。**`BAD` が残る**場合は読み込みに失敗する可能性があります（対処は第 4 章・第 6 章参照）。
...
- **3 つの引数すべて必須**です。省略すると `Syntax error` でカセットモーターも回りません。
...
`SAVE` / `SAVEM` 実行後、シミュレータは書き込み内容を**録音**しています。
...
3. 保存したファイルは、そのまま **Tape Image** にセットして読み戻せます。

> 録音内容が無いときは **録音→.t77** ／ **録音→.wav** ボタンは淡色表示になり押せません。`SAVE` を先に実行して録音を作ってください。

> 保存される WAV は、ビットを理想的な矩形波として記録したクリーンな信号（モノラル 8bit PCM）です。読み込みも安定します。
...
カセットの WRITE データ線は `$FD00` bit 0 で制御されます。WebM7 はこの信号の変化を CPU サイクル単位で
追跡し、T77 形式のパルス幅列としてバッファに**録音**しています。
**録音→.t77** ボタンを押した時点で、バッファ内容をビッグエンディアンの T77 として
シリアライズしてダウンロードします（**録音→.wav** は同じ録音を矩形波の PCM WAV にして書き出します）。

![図: SAVE 時の録音の仕組み](images/t77_05_rec_mechanism.svg)

書き込み線の High / Low の各区間の長さを計測してパルス列として蓄え、ボタンを押した時点でファイル化します。

### 3-6. 実例 1: 小さな BASIC プログラムを保存して読み戻す
...
### 3-7. 実例 2: 機械語領域を SAVEM/LOADM で往復
...
- 取り込んだ WAV をセットしたとき、品質表示が **`N blocks OK`（BAD なし）**なら成功です。**`BAD` が残る**場合は、ゲインを上げて録り直すと解消することが多いです。
...
- WebM7 の 録音→.t77 は標準的な T77 形式 (**16 バイトヘッダ + ビッグエンディアン**) で
  書き出します。LOAD 側は BE/LE を自動検出するため、旧バージョンで保存した
  リトルエンディアンのファイルもそのまま読めます。
...
| 保存した T77 が他環境で開けない | WebM7 は標準的な T77 形式（16 バイトヘッダ + ビッグエンディアン）で書き出すため、通常はそのまま読めます。 |
| 同じ SAVE 内容と新しい SAVE が混ざる | SAVE 前に **録音クリア** で録音バッファをクリア。 |
| `LOAD` 中に進捗が止まる | シミュレータがテープ読み込み時に自動高速化（CMT turbo）するので、通常は数秒で完了。停止する場合はハードウェアパネルの**リセット** → 再ロード。 |
