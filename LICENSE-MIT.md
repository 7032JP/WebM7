# 本プロジェクトの成果物のライセンス（MIT License）

本プロジェクト（WebM7）自体の成果物は MIT License で提供します。
ライセンス本文（生テキスト）はリポジトリ直下の [`LICENSE`](LICENSE) にあります。
配布物全体のライセンスの区分は、本書の「ライセンスの案内」の表が示します。

```
MIT License

Copyright (c) 2026 7032 / Naomitsu Tsugiiwa
```

## 適用範囲

MIT License は次の成果物に適用されます。

- JavaScript / HTML / CSS のソースコード（`core/`、`index.html`、`sw.js`、
  `css/` 等。ただし下記「ライセンスの案内」の表に挙げる第三者のコード・素材を除く）
- 本プロジェクトが作成したドキュメント（`README.md`、`HOWTO.md`、
  `CHANGELOG.md`、`docs/` 配下）と図（`docs/images/`）
- アイコン（`icons/`）、マニフェスト、テクスチャ（`assets/granite.png`）

## ライセンスの案内

配布物には、本プロジェクトの成果物のほかに、別のライセンスが適用される
第三者のコード・素材が含まれます。どのファイル・モジュールにどのライセンスが
適用されるかは次の表のとおりで、詳細はそれぞれの文書を参照してください。

| 範囲（ファイル・モジュール） | ライセンス | 詳細 |
| --- | --- | --- |
| 本プロジェクトの成果物（JavaScript / HTML / CSS、文書、図、アイコン、テクスチャ） | MIT License（[`LICENSE`](LICENSE)） | 本書 |
| FM 音源の移植部（`core/opn.js` の FM 合成部） | fmgen の利用条件（原文 `core/fmgen_readme.txt`） | [`LICENSE-fmgen.md`](LICENSE-fmgen.md) |
| 同梱の互換 ROM セット（`assets/altroms/`） | MIT License（別配布物。原文 `assets/altroms/LICENSE`） | [`LICENSE-AltROMs.md`](LICENSE-AltROMs.md) |
| 漢字フォント素材（互換 ROM セット内の `kanji.rom` / `kanji2.rom` に含まれる第三者素材由来の字形） | パブリックドメイン（原文 `assets/altroms/LICENSE-FONT.md` 第 4 節） | [`LICENSE-Shinonome.md`](LICENSE-Shinonome.md) |

次の要素は本プロジェクトの著作物ではなく、MIT License の対象外です。
独立したライセンス文書はなく、次節に注記としてまとめます。

## 適用範囲と注記

### 各社のブランドロゴ（`index.html`）

GitHub と X のロゴは各権利者の商標であり、公式ページへのリンク表示に使用しています。
ロゴには本プロジェクトの MIT License は適用されません。

## 商標

機種名・製品名は対応対象を示すために使用しています。本プロジェクトは各メーカーの公式製品ではなく、富士通株式会社とは一切関係ありません。
